...
trait ActiveRecord
{
    ...
    public function delete()
    {
        try {
            if (isset($this->id)) {
                $this->db->execute(
                    $this->queryBuilder->delete($this->getTable())
                        ->where('id', $this->id)
                        ->sql(),
                    $this->queryBuilder->values
                );
            } else {
                echo 'Отсутствует код удаляемой записи.';
            }
        } catch (\Exception $e) {
            echo $e->getMessage();
        }
    ...
    public function deleteAll($field, $id)
    {
        try {
            if (isset($field) && isset($id)) {
                ...
                echo 'Отсутствуют параметры удаляемых записей.';
            }
        } catch (\Exception $e) {
            echo $e->getMessage();
        }
